...
def macro_view():
    ...
    selected_ws = st.selectbox('Select the lake you would like to examine:', data["MAJOR_WATERSHED_y"].unique())
    ...
    if selected_ws:
        new_dfws = data[data["MAJOR_WATERSHED_y"] == selected_ws].set_index("seasonal.grade")
        new_dfws = new_dfws.sort_index(axis=0)
        fig_scatter = px.scatter(new_dfws,y=new_dfws.index,opacity=0.5,
                                        x = "Number Properties",color =new_dfws.index ,
                                            animation_frame="Year",animation_group="LAKE_NAME_x",
                                            facet_col="COUNTY",hover_name="LAKE_NAME_x",facet_col_wrap=2 )
